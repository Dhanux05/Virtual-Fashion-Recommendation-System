...
2. **Install required dependencies:**
   ...
   ```bash
   pip install streamlit tensorflow pandas pillow scikit-learn numpy
   ```
   
   Or install from the requirements file (if you have one):
   ```bash
   pip install -r Project_Requirements.txt
   ```

3. **Ensure required files are present:**
   - `image_features_embedding.pkl` - Pre-computed image feature embeddings
   - `img_files.pkl` - List of image file paths
   - `main.py` - Main application file

4. **Create the uploader directory (if it doesn't exist):**
   
   **Windows (PowerShell):**
   ```powershell
   if (-not (Test-Path uploader)) { New-Item -ItemType Directory -Path uploader }
   ```
   
   **Windows (Command Prompt):**
   ```cmd
   if not exist uploader mkdir uploader
   ```
...
2. **Access the application:**
   ...
   - You should see the "Clothing recommender system" interface

### Usage Instructions

1. **Upload an image:**
   - Click on the "Upload a fashion product image" button
   - Select an image file (supported formats: JPG, JPEG, PNG)
   - The uploaded image will be displayed

2. **View recommendations:**
   - The system will automatically process the uploaded image
   - It will extract features using ResNet50 neural network
   - The top 5 similar fashion items will be displayed in columns (I, II, III, IV, V)

3. **Try different images:**
   - Upload different fashion product images to see various recommendations
   - The system uses cosine similarity to find the most similar items from the dataset

### Troubleshooting

- **ModuleNotFoundError**: Make sure all dependencies are installed using `pip install`
- **FileNotFoundError**: Ensure that `image_features_embedding.pkl` and `img_files.pkl` are in the same directory as `main.py`
- **Port already in use**: If port 8501 is busy, Streamlit will automatically use the next available port. Check the terminal output for the correct URL.
- **Upload directory error**: Make sure the `uploader` directory exists in the FRS folder
...
├── image_features_embedding.pkl    # Pre-computed image features
├── img_files.pkl                    # Image file paths
├── Img_Dataset/                     # Dataset folder with fashion images
├── uploader/                        # Directory for uploaded images
